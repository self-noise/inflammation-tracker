...
{\small\color{gray}\hrule\vspace{4pt}A self-contained Progressive Web App for daily inflammation tracking, synced to your own Koofr account via WebDAV. Hosted as static files on Hostinger; runs offline; installs to phone home screen.\vspace{4pt}\hrule}
...
\texttt{sw.js}         & Service worker (offline shell cache)    & 644 \\
...
\begin{enumerate}[leftmargin=*,nosep]
  ...
  \item Entries are saved locally first, then synced as CSV rows to a path of your choosing on Koofr.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Create a folder for the data, e.g. \texttt{/inflammation/}. (Optional --- the app will create it on first submit if missing.)
...
\begin{enumerate}[leftmargin=*]
  ...
  \begin{itemize}[nosep]
    ...
    \item \textbf{CSV path on Koofr}: \texttt{/inflammation/log.csv} (or whatever you prefer).
...
\textbf{Daily entries (the actual log)} & CSV file on Koofr at your chosen path & Yes \\
\end{tabularx}
\end{center}

Only the CSV is shared across devices. Everything else is per-device.
...
\subsection{Backing up the CSV}
The CSV \emph{is} your data. Koofr offers desktop sync clients that mirror it locally. Recommended: install a Koofr desktop client and let it sync the \texttt{/inflammation/} folder to a local directory included in your usual backups.

\subsection{Inspecting the data}
\texttt{log.csv} opens directly in Excel, Numbers, pandas (\texttt{pd.read\_csv}), or MATLAB (\texttt{readtable}). The schema is:
...
\texttt{date}          & Entry date, \texttt{YYYY-MM-DD}. \\
\texttt{time}          & Entry time, \texttt{HH:MM} (24h, local). \\
...
Notes on parsing:
\begin{itemize}[leftmargin=*,nosep]
  \item All fields are CSV-escaped (wrapped in double quotes; internal quotes doubled). Excel, pandas and MATLAB handle this transparently.
...
Edit the source files locally, re-upload via SFTP or File Manager, and bump \texttt{CACHE\_VERSION} at the top of \texttt{sw.js} (e.g.\ \texttt{"v1"} $\rightarrow$ \texttt{"v2"}). This forces the service worker to refresh the cached shell on next launch.
...
\subsection*{Submit succeeds locally but the CSV never updates on Koofr}
...
\textbf{Koofr CSV path} & \texttt{/inflammation/log.csv} \\
...
\begin{enumerate}[leftmargin=*,nosep]
  ...
  \item Once authenticated, the seven app files load and the service worker caches them. The app runs entirely client-side.
  \item \textbf{On-load check:} if Koofr credentials are saved in this browser's \texttt{localStorage} and the device is online, the app silently fetches \texttt{/inflammation/log.csv}, counts rows dated today, and shows a non-blocking note (``Heads up: you already saved \dots'') above the entry form if any exist. Submission is not blocked --- duplicates are allowed.
  \item You fill in the form and tap \textbf{Save entry}. The entry is appended to a local queue in \texttt{localStorage}, then the app PUTs to your Cloudflare Worker at \texttt{koofr-proxy.\textit{\color{red!70!black}<your-account>}.workers.dev}.
  \item The Worker forwards the request to \texttt{app.koofr.net/dav/Koofr/inflammation/log.csv} with your Basic auth (email + Koofr app password) and adds CORS headers on the reply.
  \item Koofr writes the CSV row. The Worker passes the response back; the app removes the entry from the queue.
\end{enumerate}
...
Settings (per device) & \texttt{localStorage["settings"]} in each browser. Contains your Koofr email, app password, CSV path \texttt{/inflammation/log.csv}, and proxy URL \texttt{https://koofr-proxy.\textit{\color{red!70!black}<your-account>}.workers.dev}. \\
Unsynced queue (per device) & \texttt{localStorage["queue"]}. \\
Cached app shell (per device) & Service worker cache \texttt{inflam-shell-vN}. \\
\textbf{The data} & \texttt{/inflammation/log.csv} on Koofr. This is the only authoritative store --- everything else can be rebuilt from the source files. \\
\end{tabularx}
\end{center}
...
\begin{enumerate}[leftmargin=*,nosep]
  ...
  \item Open \texttt{sw.js}, bump \texttt{CACHE\_VERSION} (e.g.\ \texttt{"v2"} $\rightarrow$ \texttt{"v3"}). This forces every device to fetch the new shell on next open.
  ...
  \item Hard-reload the app on your laptop (Cmd-Shift-R) and confirm. On your phone, fully close the PWA and reopen it; you may need to do this twice on the first launch after a SW update.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*,nosep]
  ...
  \item Settings tab $\rightarrow$ \textbf{Test connection}. Failure messages map as follows:
    ...
    \end{itemize}
  \item Browser DevTools $\rightarrow$ Network: a successful PUT to the Worker should return 200 or 201 (created) on first write, 204 (no content) on subsequent writes.
