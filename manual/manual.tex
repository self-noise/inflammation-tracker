...
Open \url{https://yourdomain.example/inflam-7f3a92/} in a desktop browser. You should see the entry form: four severity buttons (1--4), a location checkbox grid, two notes fields, and a Medication fieldset with two tickboxes (methotrexate, naproxen). If you get a 403 or a directory listing, recheck folder permissions and the presence of \texttt{index.html}.
...
\texttt{naproxen}       & \texttt{0} if not taken in the past 24 hours, \texttt{1} if taken. Added 2026-06-11; absent from files written before that date. \\
\end{tabularx}
\end{center}
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item The medication columns were added partway through the project: \texttt{methotrexate} first, then \texttt{naproxen} on 2026-06-11. Entries logged before each change are missing that column and will parse as \texttt{NaN} --- treat them as ``unknown'', not ``0''. In particular, do not \texttt{fillna(0)} a medication column before analysis: that would silently recode ``not recorded'' as ``not taken'' and bias any association you compute against the drug.
  \item New columns are \emph{appended} to the end of the schema rather than inserted, so a positional reader (e.g.\ \texttt{awk -F, '\{print \$9\}'}) keeps working across the schema boundary for the columns that existed before it. Because every file carries its own header, name-based loaders (\texttt{pd.read\_csv}, \texttt{readtable}) align old and new files automatically.
