...
Each entry is its own one-row CSV file --- no header row inside any file --- with a filename of the form \texttt{<prefix>\_\_<entry-date>\_\_<write-ms>.csv}, where \texttt{<prefix>} is the basename of the log path configured in Settings (e.g.\ \texttt{log} for \texttt{/inflammation/log.csv}). To work with the log as a whole, list the folder and concatenate the matching files.

A minimal one-liner in a shell to produce a single combined CSV (run in the synced local folder; adjust the prefix as needed):
\begin{lstlisting}[language=shell]
echo 'entry_id,write_timestamp,date,time,score,locations,dietary_notes,other_notes,methotrexate' > combined.csv
cat $(ls -1 log__*.csv | sort) >> combined.csv
\end{lstlisting}

In Python with pandas:
\begin{lstlisting}[language=shell]
import glob, pandas as pd
cols = ["entry_id","write_timestamp","date","time","score","locations",
        "dietary_notes","other_notes","methotrexate"]
files = sorted(glob.glob("log__*.csv"))
df = pd.concat([pd.read_csv(f, header=None, names=cols) for f in files],
               ignore_index=True)
\end{lstlisting}
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item Per-entry files contain no header row. Supply column names when loading (\texttt{names=} in pandas, \texttt{ReadVariableNames=false} + \texttt{VariableNames} in MATLAB's \texttt{readtable}).
